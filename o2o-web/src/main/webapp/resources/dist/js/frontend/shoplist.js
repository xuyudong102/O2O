$(function () {
    ...
    //获取店铺区域列表 以及 类别列表
    // 如果有parentId 就证明是二级列表 如果没有parentId就证明是一级列表
    var searchDivUrl = "/o2o/frontend/listshopspageinfo";
    //页码
    var pageNum = 1;
    //从地址栏URL里面尝试获取parent.shopCategoryId
    var parentId = getQueryString("parentId");
    // 是否选择了子类
    var selectedParent = false; //没选择一级类别
    if (parentId){
        selectedParent = true; //选择了一级类别
    }
    ...
    function getSearchDivData() {
        ...
        $.getJSON(url, function (data) {
            if (data.success) {
                ...
                var html = "<a href='#' class='button' data-category-id='' >全部类别</a>";
        ...
        function addItems(pageSize,pageIndex){
            ...
            //设定加载符 若还在后台取数据 则不能再次访问后台 避免多次重复加载
            loading = true;
            //访问后台获取相应查询条件下的店铺列表
            $.getJSON(url,function (data) {
                if(data.success){
                    ...
                    data.shopList.map(function (item,index) {
                        html+="<div class='card' data-shop-id='"+item.shopId+"'>"+
                                    ...
                                    "<div class='card-footer'>"+
                                        "<p class='color-gray'>"+new Date(item.lastEditTime).Format("yyyy-MM-dd")+"</p>"+
                                        "<span>点击查看</span>"+
                                    "</div>"+
                                "</div>";
                    });
                    ...
                    //获取目前位置已显示的卡片总数 包含之前已经加载的
                    var total = $(".list-div .card").length;
                    //若总数达到跟按照此查询条件列出来的总数一致 则后台停止加载
                    if (total >= maxItems) {
                        // 隐藏提示符
